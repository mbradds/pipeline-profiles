...
const conditionsData = JSON.parse(getData("/drill_test/conditions.json"));
var companyResult, projectResult;

const createSeries = (data, status = "All") => {
  var companyResult, projectResult;
  [companyResult, projectResult, themeResult, counts] = groupBy(
    data,
    status,
    true
  );
  ...
  var drilldown = projectResult.concat(themeResult);

  return [seriesData, drilldown, counts];
};
